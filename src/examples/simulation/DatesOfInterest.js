 ...
 */
const adjustDateForCurrentEvent = (currentDate, currentEventIndex, shireStartDate, rivendellStartDate) => {
    if (currentEventIndex < 1 || DatesOfInterest.length <= currentEventIndex) {
        return currentDate;
    }

    return eventOfInterestToDate(DatesOfInterest[currentEventIndex], shireStartDate, rivendellStartDate);
};
 ...
 */
const findPreviousEventIndex = (currentDate, shireStartDate, rivendellStartDate) => {
    const currentSAYear = daysElapsedToSecondAgeYear(toDaysElapsed(shireStartDate, currentDate)).year;
    ...
    return DatesOfInterest.findIndex((event) => {
        ...
        return (currentDate > eventOfInterestToDate(event, shireStartDate, rivendellStartDate));
    });
};
