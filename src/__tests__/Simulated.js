...
import { getRivendellNewYearDate, makeRivendellCalendarDates } from '../RivendellReckoning';
...
it('makes correct Gondor Traditional Dates', () => {
    ...
    const yearLengthDays = 365.0 + (5/24) + (48/(24*60)) + (46/(24*60*60));
    ...
    for (let i = 0; i < 8000; i++) {
        ...
        if (calendarStewards.year <= 3441 + 3021 && (
            ...
            const displayYear = calendarStewards.year > (3441 + 1) ? `T.A. ${calendarStewards.year - 3441 - 1}` : `S.A. ${calendarStewards.year - 1}`;
            ...
            logs[displayYear] = {"Days Elapsed": daysElapsed, "Real Elapsed": realElapsed, Deficit: deficit, "Defict Formatted": formatDeficit(deficit)};

            if (calendarStewards.year === 3441+2061 || calendarStewards.year === 3441+2361) {
                appendixLogs.push(`\t${displayYear} deficit - 1day = ${roundDecimals(deficit - 1.0)} ---> ${formatDeficit(deficit - 1.0)}`)
            }
        }
    }
...
describe('print only', () => {
    ...
    const makeShireCalendar = (nextFirstDay) => (
        makeShireCalendarDates(nextFirstDay, startDateSA, GondorReckoning.RECKONING_RULES_TRADITIONAL)
    );
    const findElvesNewYear  = (elvesStartDate, shireCalendar) => {
        ...
        return shireCalendar.dates.find((date) => datesMatch(date.gregorian, elvesNewYear));
    };
    ...
    it('lists Rivendell New Year Dates on Shire Dates', () => {
        ...
        for (let year = 1; year <= 3024; year++) {
            ...
            table[row].push(`${elvesNewYear.getMonth() + 1}/${elvesNewYear.getDate()}`);
        ...
        for (let year = 1; year <= 3024; year++) {
            ...
            table[row].push(`${elvesNewYearShire.month + 1}/${elvesNewYearShire.day}`);
    ...
    it('finds Rivendell New Year Dates as Shire Dates', () => {
        ...
        [
            ...
            const elvesFirstNewYear = `${displayYear(elvesStartDate.getFullYear())}, ${elvesNewYearShire.month + 1}/${elvesNewYearShire.day}`;
            ...
            for (let i = (lastTAYear - yearRange); i < (lastTAYear + yearRange); i++) {
                ...
                const shireDisplayDate = `${elvesNewYearShire.month + 1}/${elvesNewYearShire.day}`;
                if (firstLoop) {
