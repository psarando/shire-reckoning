...
import { makeRivendellCalendarDates, getRivendellNewYearDate, TRADITIONAL_RULES, REFORMED_RULES} from '../RivendellReckoning';
import { makeGondorCalendarDates, getGondorNewYearDate, getNewReckoningNewYearDate, RECKONING_KINGS, RECKONING_STEWARDS, RECKONING_NEW } from '../GondorReckoning';
...
let weekDayString = (date, weekDay) => (`${date.toDateString()} weekDay: ${weekDay}`);
let getFirstWeekDay = (calendar) => (calendar.dates[0].weekDay);
let getLastWeekDay = (calendar) => (calendar.dates[calendar.dates.length - 1].weekDay);
...
let reckonNewYearTests = (nextFirstDay, reckonCalendar, reckonNewYearsDay, reckonFrom, reckonTo) => {
    let calendar = reckonCalendar(nextFirstDay);
    ...
    for (let i = reckonFrom; i <= reckonTo; i++) {
        for (let day = 0; day < calendar.dates.length; day++) {
            ...
            expect(firstDay.toDateString()).toEqual(getFirstDay(calendar).toDateString());
        }
        nextFirstDay = getNextDate(getLastDay(calendar));
...
describe('ShireReckoning', () => {
    ...
    it('makes correct Shire New Year Dates', () => {
        ...
        let reckonCalendar = (nextFirstDay) => (makeShireCalendarDates(nextFirstDay, startDate));
        let reckonNewYearsDay = (nextDay) => (getShireNewYearDate(nextDay, startDate));

        reckonNewYearTests(nextFirstDay, reckonCalendar, reckonNewYearsDay, -405, 405);
    });
});
...
describe('RivendellReckoning', () => {
    ...
    let reckonRivendellNewYearTests = (nextFirstDay, reckonCalendar, reckonNewYearsDay, reckonFrom, reckonTo) => {
        let calendar = reckonCalendar(nextFirstDay);
        ...
        for (let i = reckonFrom; i <= reckonTo; i++) {
            ...
            nextFirstDay = getNextDate(getLastDay(calendar));
            let nextWeekDay = weekDayString(nextFirstDay, (getLastWeekDay(calendar) + 1) % 6);

            calendar = reckonCalendar(nextFirstDay);
            expect(getFirstDay(calendar).toDateString()).toEqual(nextFirstDay.toDateString());
            expect(weekDayString(getFirstDay(calendar), getFirstWeekDay(calendar))).toEqual(nextWeekDay);
        }
    };

    it('makes correct Traditional New Year Dates', () => {
        let nextFirstDay = fullYearDate(-433,2,23);
        let reckonCalendar = (nextFirstDay) => (makeRivendellCalendarDates(nextFirstDay, startDate));
        let reckonNewYearsDay = (nextDay) => (getRivendellNewYearDate(nextDay, startDate));

        reckonRivendellNewYearTests(nextFirstDay, reckonCalendar, reckonNewYearsDay, -444, 444);
    });

    it('makes correct Traditional New Year Dates', () => {
        let nextFirstDay = fullYearDate(-433,2,23);
        let reckonCalendar = (nextFirstDay) => (makeRivendellCalendarDates(nextFirstDay, startDate, REFORMED_RULES));
        let reckonNewYearsDay = (nextDay) => (getRivendellNewYearDate(nextDay, startDate, REFORMED_RULES));

        reckonRivendellNewYearTests(nextFirstDay, reckonCalendar, reckonNewYearsDay, -405, 405);
    });
});
...
describe('GondorReckoning', () => {
    ...
    it('makes correct Stewards Reckoning New Year Dates', () => {
        let nextFirstDay = fullYearDate(-401,11,31);
        let reckonCalendar = (nextFirstDay) => (makeGondorCalendarDates(nextFirstDay, startDate));
        let reckonNewYearsDay = (nextDay) => (getGondorNewYearDate(nextDay, startDate));
    ...
    it('makes correct New Reckoning New Year Dates', () => {
        let nextFirstDay = fullYearDate(-401,11,31);
        let reckonCalendar = (nextFirstDay) => (makeGondorCalendarDates(nextFirstDay, startDate, RECKONING_NEW));
        let reckonNewYearsDay = (nextDay) => (getNewReckoningNewYearDate(nextDay, startDate));
